...
module.exports = Base.extend(Ouro.Meta, Rules, {
    ...
    return new Promise(function(resolve, reject) {
      ...
      for( var validation in list ) {
        ...
        if( list[validation].meta.async ) {
          ...
          if (response !== true) {
            ...
            return reject(new Ouro.Error('validation_error', 'A validation error occurred processing the request.', {
              status: 422,
              exceptions: [response]
            }));
          }
      ...
      if( asyncList.length > 0 ) {
        ...
        Promise.all(asyncList)
          ...
          .catch(function(err) {
            err.status = 422;
            return reject(new Ouro.Error('validation_error', 'A validation error occured processing the request.', err));
          })
      }
